...
class monitor:
    ...
    def suscribirse_a_topico(self):
        ...
        while self.is_running:
            try:
                topic, valor, fecha_hora, tiempo_inicio = socket.recv_string().split(' ', 3)
                fecha_hora = datetime.strptime(fecha_hora, "%Y-%m-%d")
                print(f"Recibido en: {topic}, Valor: {valor}, Fecha y hora: {fecha_hora}, inicio: {tiempo_inicio}")
                
                tiempo_final = time.perf_counter()
                tiempo_total = tiempo_final - float(tiempo_inicio)

                valor = float(valor)
                self.revisar_valor(valor, fecha_hora)

                print("\nValor: " + str(valor) + ", \ntiempo inicio " + tiempo_inicio + "\n tiempo final " + str(tiempo_final) +"\n Delay: " + str(tiempo_total))
    ...
    def proceso_dummy(self):
        # Simulación de un proceso dummy 108.2
        while True:
            ...
            time.sleep(5)  # Simulando un proceso que se ejecuta cada 5 segundos
